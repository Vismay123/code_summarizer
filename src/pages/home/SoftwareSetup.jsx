...
const SoftwareSetup = () => {
  ...
  // Load stored steps
  useEffect(() => {
    const storedData = localStorage.getItem("softwareInstructions");
    if (storedData) setInstructions(JSON.parse(storedData));
  }, []);

  // Save steps
  useEffect(() => {
    localStorage.setItem("softwareInstructions", JSON.stringify(instructions));
  }, [instructions]);
  ...
  return (
    <div className="w-full bg-gradient-to-br from-purple-50 via-indigo-100 to-violet-100 flex flex-col items-center py-12 px-6">
      {/* Header */}
      <div className="text-center mb-12">
        <h2 className="text-4xl font-extrabold text-indigo-800 tracking-tight drop-shadow-sm flex items-center justify-center gap-3">
          <FaTools className="text-indigo-600" /> Software Setup Steps
        </h2>
        <p className="text-gray-600 mt-3 text-lg">
          Document and track your software setup process step-by-step.
        </p>
      </div>
      ...
      {instructions.length > 0 ? (
        ...
        <div className="text-center mt-20 text-gray-600 text-lg italic">
          No software setup steps added yet. Click “Add Step” to begin!
        </div>
      )}
